...
public class CircularByteBuffer {
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	private final static int DEFAULT_SIZE = 64000;
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	protected volatile boolean inputStreamClosed = false;
	/**
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	protected volatile boolean outputStreamClosed = false;
...
//				MyPrint.printOut("CBB", "Input stream closed");
			}
		}
		...
		public int read() throws IOException {
			...
			{
				MyPrint.printOut(this.getClass().getSimpleName(), "NO ICY Stream: Cannot read");
				throw new IOException("InputStream has been closed - cannot read from a closed InputStream 2.");
			}
		...
		final static int THRESH = 2000;
		final static int MAX = 16000;
...
//				throw new IOException("InputStream has been closed - cannot read from a closed InputStream 2.");
			...
			for(;;){
					...
					synchronized(lock)
					{
						...
						{
							MyPrint.printOut(toString(), "Returned -1 in read method");
							return -1;
						}

						int available = available();
						if(available < THRESH) limit = MAX; // till avail reaches MAX no reading happens
						if (available > limit)//start reading stream
						{		
							if(limit > THRESH)limit = THRESH;		
						...
						try {							
							...
							MyPrint.printOut(toString(), e.getMessage());
						}
						if (readPosition == buffer.length) {
							readPosition = 0;
						}
...
//				MyPrint.printOut("CBB", "Output stream closed");
			}
		}
